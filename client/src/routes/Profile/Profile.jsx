        ...
        <div className={p.navContainer}>
          ...
          <div className={p.iconsContainer}>
            <Link to="/cart" className={p.cartLink}>
              <img src={korzina} alt="Корзина" className={p.cartIcon} />
            </Link>
            <Link to="/profile" className={p.profileLink}>
              <img src={Person} alt="Профиль" className={p.profileIcon} />
            </Link>
          </div>
        </div>
      </header>
        ...
        <div className={p.section}>
          ...
          {ordersStatus === 'loading' ? (
            ...
            orders.map(order => (
              ...
              <div key={order.id} className={p.orderCard}>
                <div className={p.orderHeader}>
                  <h3 className={p.orderNumber}>Заказ #{order.orderNumber}</h3>
                  <p className={p.orderTimer}>Время доставки: {timeLeft[order.id] || 'Рассчет времени...'}</p>
                </div>
          ...
          {favoriteProducts.length === 0 ? (
            ...
            <div className={p.favoritesGrid}>
              {favoriteProducts.map((product) => (
                <div key={product.id} className={p.favoriteCard}>
                  <img 
                  ...
                  <div className={p.favoriteActions}>
                    ...
                    >
                      <img src={korzina2} alt="В корзину" className={p.actionIcon} />
                    </button>
                    <button 
                      onClick={() => dispatch(toggleFavorite(product.id))} 
                      className={p.cartButton}
                    >
                      <img src={HeartFilled} alt="Удалить" className={p.actionIcon} />
                    </button>
                  </div>
                </div>
              ))}
